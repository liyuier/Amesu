      ...
      <div class="ed-col">
        ...
        <section class="ed-bottom" :style="{ height: bottomHeight + 'px' }">
          ...
          <div class="ed-bottom-body">
            <StoryCanvas v-if="state" :scene-dirs="sceneDirs" :scene-name="state?.scene || ''" :current-index="state?.index" :selected="selNode" @save="handleSceneSave" @select="(i: number) => { selNode = i; engine?.seekSceneIndex(i); }" />
          </div>
        </section>
      </div>
    </div>

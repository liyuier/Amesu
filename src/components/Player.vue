...
<template>
  <div class="ams-stage" :key="state.episode" :style="blockFx('stage')" @click="emit('advance')">
    ...
    <video v-if="state.video && state.video.src" ref="videoEl" class="ams-video" :class="{ bg: state.video.mode === 'bg' }" :src="state.video.src" :loop="state.video.mode === 'bg'" :muted="state.video.mode === 'bg'" autoplay playsinline @ended="emit('videoEnded')" @click="emit('videoEnded')" title="点击跳过"></video>

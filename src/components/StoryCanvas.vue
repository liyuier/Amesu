...
// 结点画布（Vue Flow 轻量图库）：当前场景指令 → 可拖拽/连线的横向结点图；选中→属性面板；保存回场景 JSON。
...
function sync() {
  nodes.value = props.sceneDirs.map((d, i) => ({ id: 'n' + i, position: { x: 40 + i * 200, y: 30 }, label: label(d), style: i === props.currentIndex ? { outline: '2px solid var(--accent)', outlineOffset: '2px', borderRadius: '8px' } : undefined }));
  edges.value = props.sceneDirs.slice(0, -1).map((_, i) => ({ id: 'e' + i, source: 'n' + i, target: 'n' + (i + 1), animated: true }));
}
watch(() => [props.sceneDirs, props.currentIndex], sync, { immediate: true });
function onNodeClick({ node }: { node: Node }) { selected.value = Number(String(node.id).slice(1)); }
function onDragStop() { const order = [...nodes.value].sort((a, b) => a.position.x - b.position.x).map((n) => Number(String(n.id).slice(1))); emit('save', order.map((i) => props.sceneDirs[i])); sync(); }
function onConnect(c: { source?: string; target?: string }) { edges.value.push({ id: 'e' + Date.now(), source: c.source!, target: c.target!, animated: true }); }
...
<template>
  <div class="sc-root">
    <div class="sc-bar">
      ...
      <span class="sc-hint">{{ sceneName }} · 拖拽改序 / 点结点编辑 / 连线分支；描边=当前步</span>
    </div>
    <div class="sc-flow">
      <VueFlow v-model:nodes="nodes" v-model:edges="edges" :default-viewport="{ x: 20, y: 20, zoom: 0.72 }" @node-click="onNodeClick" @node-drag-stop="onDragStop" @connect="onConnect">

...
- **剧本双轨**：同一剧本可用 **JSON 数据**（`scenes/*.json`）或 **Markdown 脚本**（`scenes/*.ams.md`；用 `##` 场景、`\`\`\`bg/bgm/video..` YAML 块、`@人物(status)` 预设动画、`>` 旁白、`*choice*` 分支等）编写，二者等价。
...
- **双轨编辑**：JSON 剧本可直接编辑校验；`.ams.md` 脚本轨改文件即热重载（dev-server 监听 → `/__reload` → 编辑器重开）；编辑器“应用”经 `irToAms` 把 IR **回写成 markdown**（无 codegen）。
- **控制栏**：播放/暂停、重播、**倍速**（对视频也生效）、交互/确定性模式、**音量滑条**、静音。
- **服务端 API**：`/api/fs/*` 自由浏览开发机目录、`/api/story` 解析 `.ams.md` 脚本轨、`/api/save` 回写剧本（JSON / `.ams.md`）、`/api/asset-list` 素材列表。
